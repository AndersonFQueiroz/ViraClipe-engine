...
def qc_day(
    ...
    try:
        for c in scored:
            ...
            # O arquivo postado é o final-*.mp4 (render); corte-*.mp4 é intermediário.
            mp4 = day_dir / f"corte-{cut_id}.mp4"
            final = day_dir / f"final-{cut_id}.mp4"
            alt = c.get("mp4")
            cands = [final, mp4] + ([Path(str(alt))] if alt else [])
            path = next((p for p in cands if p.exists()), final)
